...
avest_path = r"../resource/AvestToolbox/"

toolbox_folder_path = avest_path  # default
log_path = out_path = filter_path = ""

# options variables init

reexport = in_one_file = excel_export = do_check = do_filter = do_reload = ignore_numbers = False


def init_args(argv):
    global toolbox_folder_path, log_path, out_path, filter_path
    global reexport, in_one_file, excel_export, do_check, do_filter, do_reload, ignore_numbers

    opts, args = getopt.getopt(argv, "ce:f:p",
                               ["as-one", "reload", "ignore-numbers"])  # c: check, e: excel, f: filter, p: print,
    ...
    for arg in args:
        if arg.lower() == "mcp":
            ...
            toolbox_folder_path = aksl_path
        elif arg.lower() == "eng":
            toolbox_folder_path = eng_path
        elif arg.lower() == "toch":
            toolbox_folder_path = tocharisch_path
        elif arg.lower() == "apers":
            toolbox_folder_path = apers_path
        elif arg.lower() == "avest":
            toolbox_folder_path = avest_path

        elif os.path.isdir(arg):
            toolbox_folder_path = arg
    ...
    print(toolbox_folder_path)

    reexport = any([opt == "-p" for opt, arg in opts])
    in_one_file = any([opt == "--as-one" for opt, arg in opts]) if reexport else False
    ignore_numbers = any([opt == "--ignore-numbers" for opt, arg in opts])
    ...
    for opt, arg in opts:
        if opt == "-e":
            excel_export = os.path.join(folder_path, arg)
    ...
    do_check = any([opt == "-c" for opt, arg in opts])
    ...
    filter_path = os.path.join(toolbox_folder_path, "../resource/AvestToolbox/filter.csv") if os.path.isfile(
        os.path.join(toolbox_folder_path, "../resource/AvestToolbox/filter.csv")) else None
    if filter_path:
        do_filter = [arg.strip("\"") for opt, arg in opts if opt == "-f"]

        if do_filter:
            print("Daten werden nach folgenden Filtern gefiltert:", *do_filter)
            print("Achtung, durch die Einschränkung der gelesenen Daten wird werden alle Exporte gefiltert!")

    do_reload = any([opt == "--reload" for opt, arg in opts]) if toolbox_folder_path == mcp_path else False
    if do_reload:
...
if do_filter:
    filters = [
        [line.split(";")[0] + line.split(";")[1], line.split(";")[0] + line.split(";")[2], line.split(";")[3].strip()]
        for line in open(filter_path, "r", encoding="utf-8").readlines() if
        re.match("(.+?_[I\d]+_);(\d+)\.([\d\S]+);(\d+)\.([\d\S]+);\S", line)] if filter_path else None


def is_in_subpart(pfx_str, do_filter):
    ...
    for filter in filters:
        ...
        if (wrong_part or wrong_book or before_first_page or after_last_page or before_first_line or after_last_line):
            continue
        else:
            return filter[2]
...
def decode_toolbox_json(map, marker, prefix):  # prefix = { marker : marker_value }	 (fName, id, rec)
    ...
    if not "jumps" in markers[
        marker]:  # vgl. oben. mit dem Vorhandensein wird zwischen Id- und Record-Markern und zu interlinearisierenden Einträgen unterschieden. Bei den Text-Datenbanken ist das nativ von Toolbox vorgesehen
        ...
        for element_else in map:  # listen für ref-Gruppen
            ...
            if do_filter:
                if "ref" in prefix.keys():
                    ...
                    sub_part = is_in_subpart(prefix["ref"], do_filter)
                    if not sub_part:
                        continue
                ...
                while ii < len(decoded_table):
                    ...
                    if not tx1 == tx2:
                        if ii + 1 == len(decoded_table):  #
                            ...
                            if decoded_table[ii][
                                "tx"] == tx2:  # es fehlen lediglich Annotationen, der Rest ist identisch
                                uu += tx2.count(" ")
                                break
...
def check_word_for_consistency(word, marker):
    ...
    if do_check and not (ignore_numbers and re.match("^\d+\.?$", word[marker].strip())):
...
for text_file in text_files:
    ...
    jso = decode_toolbox_json(map, root_marker, {"fName": filename})
    print(f"Textfile {text_file}:\nMap {map} \n Json: {jso}")
    input(":)))))")
if reexport:
...
df = df.replace(r'\n', '', regex=True)
df.to_csv(out_path, sep=';', encoding="UTF-8-SIG", index=False, header=True)
...
if excel_export:
    ...
    meta_file_path = os.path.join(toolbox_folder_path, os.path.splitext(os.path.basename(excel_export))[0] + ".json")
    ...
    while True:
        try:
            wb.save(excel_export)
            break
        except PermissionError:
            input("please close " + excel_export)

    print("\n" + excel_export, "written")
    input()
...
if log:
    ...
    df.to_csv(log_path, sep=';', encoding="UTF-8-SIG", index=False, header=True)
else:
    print("no log")
